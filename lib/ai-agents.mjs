  ...
  for (const a of DEFAULT_AGENTS) {
    ...
    if (!exists) {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(a.id, a.name, a.role, a.description, a.system_prompt, a.skill, a.base_url, a.api_key, a.model, a.temperature, a.max_tokens, a.enabled, a.stream_enabled, a.vision_enabled, a.timeout_ms, a.provider_mode);
    }
  }
...
export function updateAgent(id, fields) {
  ...
  const allowed = ['name', 'description', 'system_prompt', 'skill', 'base_url', 'api_key', 'model', 'temperature', 'max_tokens', 'enabled', 'reasoning_effort', 'stream_enabled', 'vision_enabled', 'timeout_ms', 'provider_mode'];
  const sets = [];
  const vals = [];
...
function buildAgentMessages(agent, inputMessages, userContent) {
...
export async function callAgentMessages(agent, inputMessages, options = {}) {
  ...
  try {
    ...
    let data;
    try { data = await response.json(); } catch { return { error: 'API 返回异常：状态 200 但响应体不是有效 JSON（网关异常），请重试' }; }
